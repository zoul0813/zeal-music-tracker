...
void arrange_update_cell(int8_t amount) {
  ...
  switch(arrange_active_cell) {
    case Cell_Pattern: {
      ...
      if(a->pattern_index >= NUM_PATTERNS) a->pattern_index = ARRANGEMENT_OUT_OF_RANGE;
    } break;
    case Cell_Effect: {
...
void arrange_keypress_handler(unsigned char key) {
  switch(key) {
    ...
    case KB_UP_ARROW: {
      ...
      if(arrange_active_step > 0) arrange_active_step--;
      else arrange_active_step = (NUM_ARRANGEMENTS - 1);
      arrange_color_step(arrange_active_step, PATTERN_WINDOW_HL1);
      arrange_color_cell(arrange_active_step, arrange_active_cell, COLOR(PATTERN_WINDOW_HL1, TEXT_COLOR_BLUE));
    } break;
    case KB_DOWN_ARROW: {
      arrange_color_step(arrange_active_step, PATTERN_WINDOW_FG);
      arrange_active_step++;
      if(arrange_active_step > (NUM_ARRANGEMENTS - 1)) arrange_active_step = 0;
      arrange_color_step(arrange_active_step, PATTERN_WINDOW_HL1);
      arrange_color_cell(arrange_active_step, arrange_active_cell, COLOR(PATTERN_WINDOW_HL1, TEXT_COLOR_BLUE));
    } break;
    case KB_HOME: {
    ...
    case KB_KEY_TAB: {
      ...
      arrange_active_cell++;
      if(arrange_active_cell > 1) arrange_active_cell = 0;
      arrange_color_cell(arrange_active_step, arrange_active_cell, COLOR(PATTERN_WINDOW_HL1, TEXT_COLOR_BLUE));
    } break;
  }
}
